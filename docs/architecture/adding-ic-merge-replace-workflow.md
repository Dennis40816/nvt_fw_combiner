...
| Replace profile | A manifest-pinned V2 bundle under `profiles/built-in/` plus its focused `WorkbenchCompositionService.Replace.*.BuiltInV2.cs` registration | Add DP/CtrlRAM/General Replace profiles only when the IC has real range and access evidence. Synthetic legacy profiles stay contract-only. |
...
- direct V2 plan/byte assertions and owner golden coverage; retain a legacy oracle only until those checks independently cover the family
