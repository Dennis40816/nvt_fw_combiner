...
| DP Replace priority flows | no CRC/header stage unless DP evidence says otherwise | preserve or overlay TP range when DP container includes TP area | 950/951 DP Perspective Standard Merge profile is implemented with variable-length DP input padding, TP overlay `0xA000..0x36FFF`, and customer-info preservation; DP Replace still needs profile/model wiring and golden evidence | Standard Merge profile implemented; DP Replace pending |
