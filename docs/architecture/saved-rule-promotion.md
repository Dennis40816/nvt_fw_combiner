...
- General Merge saved-rule validation currently accepts only reviewed `copy-range` operation fragments, requires every mapping row to be referenced by exactly one supported operation fragment, rejects dangling slot-template references, rejects rows outside the current `output-image` / `general-output` / `reject` overlap consumption envelope, rejects unaligned rows when `alignment` is declared, rejects root or fragment processor dependencies, and treats `protectedRangePolicy` as a scalar schema enum only.
