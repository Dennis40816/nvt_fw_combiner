...
`Unsupported`. #195 assigned every remaining migration route the conservative
non-public `Internal` decision with explicit owner provenance and deleted the
legacy `Unclassified` enum/parser/UI value. Policy intake now accepts only
`Supported`, `Candidate`, `Internal`, or `TestOnly`.
`EvidenceStatus.Missing` is different: it
...
The retired migration `SupportRouteIdentity` included integrity identity/hash.
#195 removed that representation after the canonical catalog adopted the split
route-identity/capability-fingerprint contract.
...
`CapabilityFingerprint` covers the complete reviewed capability definition:
its closed allowed map variants, artifacts and parts, metadata structures,
workflow behavior, integrity routes, selection-group constraints, and compiler
semantics. The exact selected map/topology/slots and authored General values are
added by `CompilationFingerprint` on the sole `CompiledComposition`. It does not include
...
capability fingerprinting references it and adds only route and reviewed
capability-definition authority; compilation fingerprinting adds the resolved
map/topology/slot/authoring state and lowering output; run/Preview identity
...
Issue #195 proved zero repository callers and deleted
`WorkbenchCompositionService`; it was not renamed into a permanent gateway.
Focused Application contracts and Bootstrap adapters now separate capability/
session resolution, input inspection, memory projection, naming, planning,
Preview/Build execution, and typed result/report retrieval. Bootstrap owns
neither long-lived Workbench DTO semantics nor firmware policy. Architecture
tests cap focused partial aggregates and reject a replacement
`IWorkbenchEverything`, workflow-specific execution services, a second
compiler, or duplicate capability-fact owners.
...
every executable route. Issue #196 removes legacy C# built-in route lists,
legacy compilation identity/admission, and `LegacyRuntimeExecutable` after the
workflow-regression and R3 gates close. `CompiledComposition` remains the sole
execution artifact. Manifest-pinned external `combiner.exe` processors remain
separately governed adapters and are not the legacy runtime retired by this
convergence.
...
Single/Cascade routes; NT51919/NT51929/NT51932 AB Merge has no external
processor and relocates declared TP Backup scalar fields, while NT51950 and
NT51951 AB Merge use the bounded legacy Combiner processor declared by their
profiles. The IC identity alone cannot select or generalize any of these
behaviors.
...
System status is re-probed on process start. The System Information service is
the sole owner of one contract-bounded in-memory activity list for the current
process. It combines diagnostic lifecycle evidence with privacy-filtered user
operations; the default surface exposes important events and requires an
explicit Debug expansion for detailed operations. Raw paths, filenames, user
identifiers, and multiline values are rejected. Resolved issues stop
...
4. `PublicationStatus` is the independently versioned support claim, including
   ...
   policy is a materialization failure, not a fifth publication value; the
   retired `Unclassified` token is rejected by canonical policy intake.
...
| `PendingInput` | A required user-supplied artifact or selection is absent. | Preserve already resolved sibling facts, name the exact next action such as `Load TP first`, and expose `CanSelect = false` for an independent dependent-slot transition. |
...
ADR 0048 keeps this prerequisite/readiness dimension separate from the
selected-artifact inspection lifecycle. A ready selected artifact moves through
transient `Checking` to terminal `Verified`, `Warning`, or `Error`, bound to the
current compilation and file identity. `Ready` is therefore not a synonym for
`Verified`, and Presentation cannot manufacture either result.
...
For NT51950 with TP absent, the topology-dependent DP child and Initial Code
address map are `PendingInput(TP.FWConfig.ChipNumber)`, expose
`CanSelect = false`, and label the unresolved placement **Load TP first**. An
independent UI/session transition cannot add or replace DP until TP is ready;
an atomic headless request containing both remains order-independent. If a
previously coherent session loses TP, any retained DP identity may remain
visible but its terminal health is invalidated and Build is unavailable.
Supplying a valid TP replaces the snapshot with selectable/ready dependent
children. Supplying an invalid TP instead produces `Blocked` issues and a
reportable non-executable result; it does not guess the DP placement.
