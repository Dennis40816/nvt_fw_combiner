...
| `v0.9.9` | Annotated tag object `1590a27f…28bd8f96` peels to protected-main commit `32c37e25`; its tree contains `VERSION=0.9.9` | PR `#149`, required main/package/release CI, and the GitHub Release are complete | Published stable release and the only authoritative `v0.9.10` predecessor |
...
| `0.9.9` integration correction | PR `#148` changed only `VERSION`, `SPEC.md`, `CHANGELOG.md`, and verification records | Completed through `0.9.9.5`, then PR `#149` to protected `main` | Historical integration step that produced the published `v0.9.9` authority above |
...
tag, and requires its commit to be reachable from `main`. Published `v0.9.9`
satisfies that rule at `32c37e25`. The four-component `v0.9.9.5` tag remains an
internal consolidation marker and must not be dispatched through
`.github/workflows/release.yml` or represented as a downloadable stable release.
...
includes product/project Markdown below `docs/` plus the root specification,
changelog, and README; repository agent-instruction files such as
`docs/AGENTS.md` are excluded. These are physical tree measurements, not
`git diff` additions.
...
Owner decision on 2026-07-20 raises the temporary `v0.9.10` production ceiling
from 60,000 to 60,100 so release-blocking firmware-identity and final
save-dialog revalidation fixes are retained instead of deleting safety or
evidence to meet a source target. The reviewed tree measures 60,042 nonblank
production lines. Named exact partial ratchets are
`WorkbenchCompositionService` 4,419 and `MainWindowViewModel` 4,177. Exact JSON
...
- Publishes a typed committed-output boundary after atomic Build promotion while
  complete report, Hex Diff, and history projection finish off dispatcher; the
  report remains complete and the run retains command ownership until ready.
...
Snapshot: 2026-07-20 11:00 Asia/Taipei. Branch progress may advance after this
...
| `v0.9.10` stable reconciliation | Complete | PR `#150` integration baseline `e968a310`; later reviewed fixes remain on the current version branch | Preserve the complete current `0.9.10` tree through the protected-main integration |
| `v0.9.10` authoritative B/C | Complete | B `32c37e25`, C `6f3698dd`; exact hashes/counts/raw samples in the performance baseline, with later UAT/safety fixes covered by focused parity and the canonical verifier | Preserve the recorded comparison boundary; do not recast later release fixes as new performance samples |
| `v0.9.10` publication | Version integration complete | The current `0.9.10` integration branch is authoritative; VERSION is `0.9.10` | Bind the immutable SHA only after final PR/review closure, then complete exact-main package/provenance, annotated tag, and GitHub Release |

The stable-predecessor B/C comparison remains fixed at Node B `32c37e25` and
Node C `6f3698dd`. PR #150's integration baseline at `e968a310` and the later
current-branch fixes are separately reviewed; the immutable release authority
is the eventual protected-main merge commit recorded by package provenance.
Protected-main integration, exact-main packaging, tag, and GitHub Release
...
- Restore descending total-source and partial-aggregate ratchets from the
  reviewed result. Do not treat unused capacity below 60,100 as a permanent
