...
The raw `v0.9.9.5..437dffaf` tree comparison is `+7,808` production nonblank
lines, but that is not the expected integrated result. The source branch does
not yet contain the final predecessor's compiler retirement and canonical
consolidation, so that comparison includes approximately 2,010 lines of branch
divergence. Applying the isolated stack delta arithmetically to `v0.9.9.5`
would produce about 59,426 production lines. This is an estimate only: the
final rebase and conflict reconciliation must be measured again before review.

Both the raw 61,436-line tree and the approximately 59,426-line integrated
projection exceed the current 55,031-line `v0.9.10` policy gate. Source size is
not a performance KPI, but the release cannot silently raise or delete this
ratchet. The final tree requires an explicit maintainability decision backed by
an inventory of retained safety, accessibility, evidence, and performance code.
...
- Remains pre-release: the final predecessor rebase, authoritative B/C capture,
  source-ratchet decision, review, canonical verification, required CI, main
  merge, tag, and release are still gates.
...
Snapshot: 2026-07-19 16:30 Asia/Taipei. Branch progress may advance after this
...
| `v0.9.9` code milestone | 9/9 | Immutable tag at `270e803e` | Historical tag retains `VERSION=0.9.8` and is not reused |
| `v0.9.9.5` consolidation | 8/8 | Integration/tag commit `0589ba3a` | Internal predecessor only; no stable package is built from the four-component tag |
| `0.9.9` metadata alignment | CI complete, review/merge pending | PR `#148`, head `7fe30135`; all three required CI jobs passed | Merge through `0.9.9.5`, then reviewed integration to `main`; do not move historical tags |
| `v0.9.10` release checklist | 0/9 formally closed | Frozen source stack `437dffaf`; reconciliation is split into nine semantic batches | Complete predecessor reconciliation, B/C, parity, review, verifier, CI, main merge, tag, and release |

The `v0.9.10` implementation is materially advanced even though its formal
release checklist remains at zero: checklist items are counted only after the
final predecessor is integrated. Conflict resolution is commit-by-commit for
traceability, but tests run at semantic batch boundaries and once at the final
canonical gate rather than after every commit.
