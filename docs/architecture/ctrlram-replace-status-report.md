...
> production capabilities under `SPEC.md`; any route, interval, plan, or result
> below for those ICs is historical characterization only. #177, #187, and
> #194 must not migrate it, and #221 removes every production selector,
> registration, package entry, and publication row for those ICs.
...
evidence only. [ADR 0031](../adr/0031-ctrlram-profile-intervals-and-build-plan-authority.md)
further supersedes exact PID/version/count route claims: family comes only from requested IC,
...
selection. The pre-#221 compatibility baseline's 31 modeled runtime
interval/build-plan pairs have explicit trusted V2 routes. This includes
NT51928 non-NB single/2-chip/3-chip through the matching NT51927 TP plans and
...
```text
NT51926 has two runtime profile intervals: `[1.0.0, 2.0.0)` uses the 1.4.1-sourced profile and `[2.0.0, infinity)` uses the 2.0.0-sourced profile. NT51930 has one runtime profile sourced from 1.4.0, so it covers `[1.0.0, infinity)`; its inspected 2.0.0 BAT remains evidence-only and creates no production boundary.
```
...
| --- | --- | --- | --- |
| NT51917 | NT51927 perfect-family alias: `MERGE_MODE` + `NT51927BASED_GEN_CRC_MODE CRC32` | owner alias confirmation plus NT51927 exact single/two-/three-chip fixtures | The three fixtures preserve V1/V2 bytes and 7/10/13-command evidence for the single/2-chip/3-chip plans. Hash and fixture metadata are evidence only; no support promotion is made. |
...
| NT51932 | `NT51932BASED_NORMAL_MODE CRC8` | inspected BAT + final 2026-07-18 direct AUTO_PRJ-525 intake | The PID `0x5601`/Common FW 2.0.0/cascade-3 fixture proves V1/V2 full-byte parity. The direct NF composite equals `NF_Diff_0.bin`; no DiffNFMerge derivation is claimed and fixture metadata does not narrow the bounded 2–8 cascade route. |
| NT51950 | `NT51950BASED_NORMAL_MODE CRC8` | 2026-07-17 direct AUTO_PRJ-676 single plus owner-approved #188 Cascade geometry alias | Single retains V1/V2 SHA `a32e6896...d5c4`. Exact-2-IC Cascade reuses only the NT51951 direct case's TP geometry/postbuild facts: write `[0x33200,0x33B10)`, preserve `[0x33B10,0x34600)`, and use fixed FWConfig Backup `0x36000`. The `0x80000` NT51951 output is not an NT51950 direct golden; NT51950 remains `0x40000`. |
| NT51951 | NT51950 postbuild flow | final 2026-07-18 direct single plus 2026-07-31 direct AUTO_PRJ-599 exact-2-IC Cascade intake | Single retains exact-case V1/V2 parity. Cascade locks the complete `0x80000` owner expected, masked DiffDLM placement, preserved active Diff NF/inactive target, fixed FWConfig Backup, and only four approved CRC-word differences from registered Combiner 1.13.0. Inactive source uniform-fill is golden-only. |
...
- A 2026-07-06 TP Overview end-flag audit checked the row immediately before each `end_flag (0x00,N,V,T)` section:
  ...
  | NT51950 / NT51951 | FW Information for Host / fixed FWConfig Backup envelope before End Flag | `[0x36000,0x36FFC)` | legacy `fw-information-host` display row; canonical Backup starts at `0x36000` because the fixed End Flag begins at `0x36FFC` |
...
- Tool/mode compatibility is closed. Runtime remains fail-closed only until the
  required V2 profile/plan coverage and independent R3 review are complete.
...
- It does not answer the real NT51926 issue. NT51930's evidence-only 2.0.0 shape cannot execute, while its sole runtime profile already covers every supported Common FW version; prefill does not affect that selection.
...
If owner later insists on production behavior, treat it as R3 firmware behavior: add an ADR/contract, declare the prefill as an ordered profile operation over a half-open range, require matching owner golden expected outputs for each runtime-profile interval, and prove wrong-profile selection fails closed.
...
| NT51932 cascade-3 golden case | AUTO_PRJ-525/PID `0x5601`/Common FW 2.0.0/cascade 3 identifies the fixture. The pre-retirement V1 control and V2 output are byte-identical at SHA `0e59a2fb...2566`; their only 16 differences from owner expected are four approved CRC words. One session runs the two BAT-ordered registered Combiner 1.13 commands. Its active-record end is `0x2F900`; the unique NVT marker resolves the Backup start to the expected aligned `0x30000`. | Engineering parity is closed for the historical fixture. The `0.10.x` correction separately replaces only the two active DLM prefixes, preserves active NF and inactive records, and treats postbuild Backup placement mismatch inside declared authority as a Build Report warning. The reference hash, PID, version, and count remain evidence; production admission uses the IC's sole runtime profile and bounded 2–8 cascade plan. DiffNFMerge is neither executed nor claimed. |
...
| NT51950/NT51951 cascade scope | Owner confirms each IC's single/cascade TP layout, flash offsets, and postbuild command contract are identical. The current confirmed Cascade shape is 2 IC with one record at `0x33200`: Diff CtrlRAM `[0x33200,0x33B10)` is writable and Diff NF `[0x33B10,0x34600)` is reference-preserved. The map fixes the End Flag at `0x36FFC`, so the canonical FWConfig Backup starts at `0x36000`; postbuild copies the primary FWConfig from `0x22200` to that fixed destination. Existing real-tool smoke covers the legacy single/cascade invocation for both ICs. NT51950 retains a `0x40000` container and NT51951 a `0x80000` container. LDC is already packaged in each DP payload rather than exposed as the separate section used by NT51928. | The `0.10.x` migration must ignore inactive AE dummy content, preserve NF, enforce the fixed Backup postcondition, and must not infer wider counts or the NT51929-family count-derived placement formula. Direct expected-output and exact mutation evidence remain promotion gates; AB remains a separate workflow. |
