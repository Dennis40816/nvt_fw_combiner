...
- Firmware-owner decisions recorded on 2026-07-13: legacy workflow is the frozen V2 migration baseline, DP Replace customer information follows replacement DP, and the committed legacy output hashes are accepted for V2 runtime promotion. `knownDeviations` is empty, so parity remains full-byte exact; a future deviation requires a separate R3 record with a declared half-open range, reason, evidence, and approver. This does not claim independent hardware validation.
