...
| NT51926 | reference candidate | no evidence | DP/CtrlRAM/General TBD | CtrlRAM Replace postbuild category selected by FWConfig Common FW version: `1.4.1` -> `0x32F50`, `2.0.0` -> `0x32A70` | `gen_flash_bin_v2` config + 2026-07-05 CtrlRAM fixture; expected output and versioned TP Overview rows pending | Candidate |
...
| NT51930 | flash-map derived Standard Merge profile | no evidence | CtrlRAM priority; DP/General TBD | Standard Merge golden from `merge_bin.7z`; CtrlRAM Replace uses `NT51930BASED_NORMAL_MODE CRC8`; FWConfig Common FW `1.x.x` selects the archived `1.4.0` command shape, while `2.0.0` selects the newer two-command shape | IC FlashMap postbuild + owner IC-count confirmation; CtrlRAM expected output, versioned TP Overview rows, and firmware-owner sign-off pending | Candidate; postbuild core implemented |
