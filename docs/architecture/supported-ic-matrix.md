...
| P1 | Report/history completeness | Preview/Build report modal exists; first-peak errors, save flow, output artifact path display, local persistent report history, and structured operation evidence cards for status, overlap, normalized ranges, processor/tool id, read/write ranges, and Combiner argv exist. | Finish richer persisted history metadata plus explicit warning/gated-state presentation and support-ready audit review. |
