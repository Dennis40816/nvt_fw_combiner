...
> production capabilities under `SPEC.md` and #221. Any row below that still
> names one of them, or says `all 13`/`all 31`, characterizes the executable
> pre-#221 compatibility baseline or historical evidence only. It is not
> target admission, publication, or migration authority. #177, #187, and #194
> must exclude those four ICs; #221 removes their selectors, routes,
> registrations, package entries, and Support Matrix publication rows.
