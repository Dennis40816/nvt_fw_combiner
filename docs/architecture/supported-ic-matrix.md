...
| P1 | Report/history completeness | Preview/Build report modal exists; first-peak errors, save flow, output artifact path display, local persistent report history, structured operation evidence cards for status, overlap, normalized ranges, processor/tool id, read/write ranges, and Combiner argv exist. Known truncation diagnostics render as warnings rather than blocking issues. | Finish richer persisted history metadata, schema-level severity, gated-state presentation, and support-ready audit review. |
