...
Standard Merge and DP Replace use the same canonical IC memory-map facts. Therefore, an IC cannot expose DP Replace unless it also exposes Standard Merge. The repository retains 13 public Standard Merge IC routes and 13 public DP Replace registrations. NT51928 additionally has one isolated, hidden no-LDC Standard Merge variant and two hidden partial DP Replace fixed-input variants so the existing trusted `0x80000` Standard Merge and DP+LDC bundle identities remain unchanged. The partial variants reuse the canonical NT51928 DP Replace family/map and do not create additional public routes or support claims. For the focused v0.9.17 selector policy, 10 ICs are user-selectable; NT51920, NT51930, and NT51931 remain internally modeled for compatibility but are omitted from every user-facing IC selector. NT51925 has no current catalog row. This is an executable authoring statement, not automatic product-support promotion; firmware-owner release review remains separate.
...
- NT51928 non-NB: independently selectable Initial Code `[0x3C000, 0x40000)` and LDC `[0x40000, 0x62000)` in an exact `0x80000` base; at least one must be selected.
...
| P0 | DP Replace production closure | All 13 retained IC rows have executable trusted V2 profiles, canonical range tests, and full-output same-build controls; the v0.9.17 user selector exposes 10 of them. NT51928 alone has independently optional Initial Code/LDC inputs with at least one required. NT51950/NT51951 retain their deterministic capacity oracle and archived comparison; other routes remain Evidence open in the catalog. | Firmware-owner release/support selection and any independently required direct DP Replace output packages. Non-selectable NT51920/NT51930/NT51931, No-Jira NT51930 content, and NT51928 NB remain outside this admission. |
...
| NT51928 | canonical V2 Standard Merge with optional LDC: `0x40000` without LDC, existing `0x80000` with LDC | no evidence | DP Replace requires exact `0x80000` Reference and at least one of Initial Code `[0x3C000,0x40000)` or LDC `[0x40000,0x62000)`; CtrlRAM exposes non-NB single/2-chip/3-chip plans | Standard Merge selects only by LDC binding presence and fails closed for an invalid supplied LDC. DP Replace validates the complete selected set atomically and preserves every unselected Reference byte. CtrlRAM reuses the matching NT51927 TP layout and postbuild branch inside distinct 512 KiB maps; `[0x34800,0x80000)` stays preserved. NT51928 NB remains excluded. | direct `gen_flash_bin_v2` NT51928 full output + complete `0x40000` prefix parity + changed-input Initial/LDC preservation oracles + owner-approved NT51927 geometry + manifest-pinned routes + two-chip CtrlRAM parity | Support-neutral non-NB routes; single/three-chip independent output review remains a promotion gate. |
...
- DP Replace: all 13 retained IC rows have executable canonical maps and whole-part atomicity, while the v0.9.17 user selector exposes 10. Same-build controls plus NT51928 changed-input oracles prove full-output equality, selected-part replacement, and unselected Reference preservation; catalog Evidence-open states still require firmware-owner release review and any independently requested direct output package. Non-selectable NT51920/NT51930/NT51931, No-Jira NT51930 content, and NT51928 NB are not admitted evidence rows;
