...
- include NT51950 and NT51951 normal Merge with the confirmed DP Perspective TP overlay range `0x0A000-0x36FFF (len 0x2D000)`; golden cases remain required;
...
| P0 | Standard Merge golden closure | Golden-backed gen_flash profiles are executable; NT51917/NT51919 are executable owner-confirmed aliases and are tested against NT51927/NT51929 golden bytes. | Owner-approved golden outputs for NT51930 and NT51950/NT51951, optional direct NT51917/NT51919 alias samples if desired, plus firmware-owner sign-off for every released Standard Merge profile. |
...
| NT51930 | flash-map derived Standard Merge profile | no evidence | CtrlRAM priority; DP/General TBD | CtrlRAM Replace uses `NT51930BASED_NORMAL_MODE CRC8`; current cascade maps to `<=13 IC` DiffDLM branch | IC FlashMap postbuild + owner IC-count confirmation; normal merge golden pending | Candidate; postbuild core implemented |
...
| NT51950 | normal merge requested; executable DP Perspective profile | uploaded combiner; deferred | DP and CtrlRAM priority | CtrlRAM Replace postbuild uses `NT51950BASED_NORMAL_MODE CRC8`; Standard Merge accepts only `0x40000`/`0x80000`/`0x100000` DP inputs; DP Replace workbench path requires exact `0x100000` base, accepts shorter replacement DP with padding, then restores TP `0x0A000-0x36FFF (len 0x2D000)` | IC FlashMap postbuild + DP Perspective; normal merge golden pending | Priority candidate pending golden |
| NT51951 | normal merge requested; executable DP Perspective profile shared with 950 | uploaded config; deferred | DP and CtrlRAM priority | CtrlRAM Replace uses the NT51950 reference flow; Standard Merge and DP Replace share the same confirmed 950/951 DP length and TP restore policy | DP Perspective evidence + owner alias confirmation; normal merge golden pending | Candidate pending golden |
