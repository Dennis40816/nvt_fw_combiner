...
- open the `v0.9.15` AB runtime/UI/CLI function only for NT51919/NT51929/NT51932 under the owner-approved fixed no-processor plan; missing direct golden evidence is recorded as support-certification debt rather than a function gate, while NT51950/NT51951 stay closed for their separate non-golden R3 authority gaps;
...
Standard Merge and DP Replace use the same canonical IC memory-map facts. Therefore, an IC cannot expose DP Replace unless it also exposes Standard Merge. As of the owner-approved v0.9.11 R3 phase, all 13 selectable Standard Merge ICs have isolated trusted DP Replace bundles, runtime registrations, and profile-derived UI/CLI inputs. This is an executable authoring statement, not automatic product-support promotion; firmware-owner release review remains separate.
...
| AB Merge function | NT51919, NT51929, NT51932 | `nt51919-nt51929-nt51932-ab-merge` trusted V2 bundle, `IcSupportCatalog`, shared Application runner | `0.9.15` opens the fixed 512 KiB DP_AB plus two 256 KiB TP-input route in UI and CLI. The six-operation plan copies DP_AB, overlays TPA, relocates only TPB `0x7164/0x7168/0x716C` by `+0x40000`, then overlays bank B. Short sources block; oversized sources warn and expose full-source plus accepted-prefix report identities. NT51929 has direct golden parity; NT51919/NT51932 retain fact-scoped evidence debt. | Missing direct golden alone does not close this function. Support certification still requires firmware-owner confirmation of the no-processor plan, classified legacy header/CRC differences, Release EXE smoke, final clean verification, release-owner approval, and the direct-golden debt ledger below. |
...
| Production Replace profiles | All 13 selectable ICs route trusted V2 DP Replace profiles; CtrlRAM workbench profiles are generated from FlashMap/postbuild evidence; General Replace UI/CLI workbench supports DP-kind mappings and TP/CtrlRAM mappings with postbuild refresh | `WorkbenchCompositionService`, isolated hash-pinned DP Replace bundles, hash-pinned FlashMap/Postbuild config | Gen Flash profiles clone the exact declared base and replace only the DP ranges listed above; NT51928 additionally requires an independent LDC FlashCode input and operation. NT51930 retains `[0x0000, 0x6000)`. NT51950/NT51951 retain capacity-selected container replacement plus TP restore/customer-from-DP behavior. UI/CLI slots come from the compiled profiles, and full-output self-replacement parity covers the applicable owner-provided Standard Merge controls without changing existing Merge bundle identities. CtrlRAM and General Replace behavior is unchanged. | Independent firmware-owner release review/support selection; direct DP Replace output evidence where the owner requires it beyond the admitted same-build controls; real CtrlRAM/General Replace profiles for released modes; TP-touching General Replace golden outputs; and finalized owner-approved safety envelopes. |
...
| NT51950 one-IC route | Closed candidate (`0.9.16`) | Present: owner-supplied single-topology evidence | Firmware-owner approval of the exact staged Combiner sequence. |
| NT51950 two-IC route | Closed candidate (`0.9.16`) | Missing | Independent topology/map/processor authority and firmware-owner approval. |
| NT51951 one-IC route | Closed candidate (`0.9.17`) | Missing | Independent topology/map/processor authority and firmware-owner approval. |
| NT51951 two-IC route | Closed candidate (`0.9.17`) | Missing | Independent topology/map/processor authority and firmware-owner approval. |

| Measured progress | Calculation | Current value |
| --- | --- | --- |
| AB function availability by target IC | 3 open ICs (NT51919/NT51929/NT51932) / 5 planned AB ICs | **60.0%** |
| Direct AB-golden coverage | 2 present cells (NT51929 fixed, NT51950 one-IC) / 7 planned IC/topology cells | **28.6%**; **5 missing** |
| Direct AB-golden debt | 5 missing cells / 7 planned IC/topology cells | **71.4%** |
| `0.9.15` delivery checklist | 4 complete items (function opening, output naming, TPA/TPB identity inspection, silent perfect-family reconciliation) / 5 items including delivery-to-review automation | **80.0%** |
| AB support certification | 0 fully certified cells / 7 planned IC/topology cells | **0.0%**; function availability must not be presented as certification. |

The five missing direct AB golden cells are NT51919 fixed, NT51932 fixed,
NT51950 two-IC, NT51951 one-IC, and NT51951 two-IC. These counts are AB Merge
only; Standard Merge, DP Replace, and CtrlRAM evidence are not substituted into
this ledger.
...
| P0 | DP Replace production closure | All 13 selectable ICs have executable trusted V2 profiles, profile-derived UI/CLI slots, canonical range tests, and full-output same-build controls. NT51928 alone has a required separate LDC input. NT51950/NT51951 retain their deterministic capacity oracle and archived comparison; other routes remain Evidence open in the catalog. | Firmware-owner release/support selection and any independently required direct DP Replace output packages. No-Jira NT51930 content and NT51928 NB remain outside this admission. |
...
| P2 | Deferred workflows | The NT51919/NT51929/NT51932 AB runtime/CLI pilot is admitted support-neutrally; its minimum UI and release gates remain open. NT51950/NT51951 AB, UI Saved Rules, General profile promotion, General Replace saved-rule execution, and future REG Replace remain deferred. | Close the pilot's firmware-owner/UI/release gates independently; promote NT51950/NT51951 only after their distinct processor/integrity evidence and owner priority. |
...
| NT51919 | canonical V2 Standard Merge route; map-bound alias of NT51929 | support-neutral fixed `0x80000` runtime/CLI pilot through the owner-approved NT51929 fact scope | DP Replace plus one `[1.0.0,infinity)` CtrlRAM profile with single and generic cascade plans | AB uses the exact six-operation no-processor plan; CtrlRAM single regression uses the NT51929 family with NT51919 processor identity and cascade adds DiffDLM `[0x2D100,0x35D00)`. | owner perfect-family confirmation + direct NT51929 AB golden/fact scope + manifest-pinned NT51919 profiles; independent final firmware/release review remains | Support-neutral routes; no public product-support promotion. |
...
| NT51929 | canonical V2 Standard Merge route | support-neutral fixed `0x80000` runtime/CLI pilot with direct owner golden parity | DP Replace plus one `[1.0.0,infinity)` CtrlRAM profile with single and generic cascade plans | AB has no processor and relocates only the three declared TPB scalars; CtrlRAM single locks registered Combiner 1.13 two-command parity and cascade adds DiffDLM `[0x2D100,0x35D00)`. | direct owner AB golden + trusted V2 AB/single/cascade profiles; classified legacy header/CRC differences and final release review remain explicit gates | Support-neutral routes; no public product-support promotion. |
...
| NT51932 | canonical V2 Standard Merge route | support-neutral fixed `0x80000` runtime/CLI pilot through the owner-approved NT51929 fact scope | DP Replace plus one `[1.0.0,infinity)` CtrlRAM profile with single and generic cascade plans | AB uses the same exact six-operation no-processor plan; CtrlRAM single uses NF/Normal/VN and cascade adds DiffDLM `[0x2D100,0x35D00)`. | direct cascade regression + owner-approved NT51929 AB family facts; independent final firmware/release review remains | Support-neutral routes; no public product-support promotion. |
| NT51950 | canonical V2 DP Perspective Standard Merge route, exact-capacity selected | repository-only AB V2 candidate; application run requests are rejected | DP and CtrlRAM priority: DP Replace has LDC already packaged in DP; `[1.0.0,infinity)` CtrlRAM single and generic cascade plans | DP packages LDC in its payload. CtrlRAM single/cascade share the TP layout, offsets, `NT51950BASED_NORMAL_MODE CRC8`, and DiffDLM `[0x33200,0x34600)`; the full image ends at `0x40000`. | direct single golden, single/cascade real-tool smoke, owner layout-equivalence decision; cascade stays support-neutral | Priority candidate; AB remains separate and unpromoted. |
| NT51951 | canonical V2 DP Perspective Standard Merge route, exact-capacity selected | distinct AB V2 candidate; application run requests are rejected | DP and CtrlRAM priority: DP Replace has LDC already packaged in DP; `[1.0.0,infinity)` CtrlRAM single and generic cascade plans | DP packages LDC in its payload. CtrlRAM TP layout, offsets, command mode, and DiffDLM match NT51950; only the preserved image tail extends to `0x80000`. | direct single golden, single/cascade real-tool smoke, owner layout-equivalence decision; cascade stays support-neutral | Candidate; AB remains separate and unpromoted. |
...
- DP Replace: all 13 selectable ICs now have executable canonical maps and whole-part atomicity. Same-build FlashCode/Initial Code controls prove full-output equality for every admitted route, including separate NT51928 DP/LDC writes; catalog Evidence-open states still require firmware-owner release review and any independently requested direct output package. No-Jira NT51930 content and NT51928 NB are not admitted evidence rows;
