...
| Production Replace profiles | NT51950/NT51951 DP Replace workbench path; CtrlRAM workbench profiles are generated from FlashMap/postbuild evidence; General Replace UI/CLI workbench supports DP-kind mappings and TP/CtrlRAM mappings with postbuild refresh | `WorkbenchCompositionService`, `BuiltInReplaceProfiles`, `TpFlashMapCatalog` | NT51950/NT51951 DP Replace builds by cloning an exact-length base of `0x40000`, `0x80000`, or `0x100000`, replacing the padded selected-length DP container, then restoring TP `0x0A000-0x36FFF (len 0x2D000)`. CtrlRAM workbench runs selected region slots through staged postbuild. General Replace can build explicit mappings inside cataloged DP-kind ranges from UI rows or repeated CLI `--mapping <target-start+length=path>`; TP/CtrlRAM-touching mappings append the selected legacy Combiner postbuild refresh after the explicit mapping. Header/customer/project/backup rows remain protected. Built-in Replace profiles remain contract evidence. | Real DP/CtrlRAM/General Replace profiles are promoted per released IC/mode, TP-touching General Replace golden outputs are signed off, and owner-approved safety envelopes are finalized. |
