...
| NT51950 | normal merge requested; executable DP Perspective profile | uploaded combiner; deferred | DP and CtrlRAM priority | CtrlRAM Replace postbuild uses `NT51950BASED_NORMAL_MODE CRC8`; Standard Merge accepts variable DP input up to `0x100000`; DP Replace workbench path requires exact `0x100000` base, accepts shorter replacement DP with padding, then restores TP `0x0A000..0x36FFF` | IC FlashMap postbuild + DP Perspective; normal merge golden pending | Priority candidate pending golden |
