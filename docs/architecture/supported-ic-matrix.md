...
| NT51919 | canonical V2 Standard Merge route; map-bound alias of NT51929 | fixed `0x80000` V2 candidate with an explicit fact-scoped NT51929 region-set alias; application run requests are rejected | DP Replace plus one `[1.0.0,infinity)` CtrlRAM profile with single and generic cascade plans | Single regression uses the NT51929 family with NT51919 processor identity; cascade adds DiffDLM `[0x2D100,0x35D00)`. Exact fixture metadata is not admission authority. | owner perfect-family confirmation + direct NT51929 fixture + manifest-pinned NT51919 profiles; independent cascade output review remains a promotion gate | Support-neutral CtrlRAM alias; no public support promotion. |
...
| NT51929 | canonical V2 Standard Merge route | fixed `0x80000` V2 AB candidate; application run requests are rejected | DP Replace plus one `[1.0.0,infinity)` CtrlRAM profile with single and generic cascade plans | Single regression locks registered Combiner 1.13 two-command parity; cascade uses the same physical regions plus DiffDLM `[0x2D100,0x35D00)`. AB remains separate. | owner-approved non-AB single fixture + trusted V2 single/cascade profiles; independent cascade output review remains a promotion gate | Support-neutral CtrlRAM candidate; no public support promotion. |
...
| NT51932 | canonical V2 Standard Merge route | fixed `0x80000` V2 AB candidate; application run requests are rejected | DP Replace plus one `[1.0.0,infinity)` CtrlRAM profile with single and generic cascade plans | Single uses NF/Normal/VN; cascade adds DiffDLM `[0x2D100,0x35D00)`. AB TPA/TPB behavior remains separate. | direct cascade regression + owner-approved NT51929 family facts; independent single output review remains a promotion gate | Support-neutral DP/CtrlRAM candidates; AB remains unpromoted. |
