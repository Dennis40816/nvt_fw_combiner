...
Current owner priority as amended on 2026-08-24:

- keep DP Replace profiles and executable regression semantics but hide its
  ordinary UI/CLI authoring surface in `0.10.7` and the initial `1.0.0`; decide
  retirement or reopening at `1.1.0`;
- continue normal Merge and CtrlRAM Replace as ordinary authoring workflows;
...
`1.9.0` makes every DP Replace authoring route unavailable, so `0.10.7` and the
initial `1.0.0` do not enumerate the workflow in ordinary UI/CLI authoring.
