...
| Production Replace profiles | NT51950/NT51951 DP Replace workbench path; CtrlRAM workbench profiles are generated from FlashMap/postbuild evidence; General Replace UI/CLI workbench supports DP-kind explicit mappings only | `WorkbenchCompositionService`, `BuiltInReplaceProfiles`, `TpFlashMapCatalog` | NT51950/NT51951 DP Replace builds by cloning an exact-length base of `0x40000`, `0x80000`, or `0x100000`, replacing the padded selected-length DP container, then restoring TP `0x0A000-0x36FFF (len 0x2D000)`. CtrlRAM workbench runs selected region slots through staged postbuild. General Replace can build explicit mappings inside cataloged DP-kind ranges from UI rows or repeated CLI `--mapping <target-start+length=path>`, protects header/customer/project/backup rows, and fails closed for TP/CtrlRAM ranges until approved Combiner refresh is wired. Built-in Replace profiles remain contract evidence. | Real DP/CtrlRAM/General Replace profiles are promoted per released IC/mode, TP-touching General Replace gets processor wiring, and golden outputs are signed off. |
...
| P0 | General Replace production closure | Workbench Preview/Build can run explicit mappings inside cataloged DP-kind ranges and reports compiler issues for protected or TP/CtrlRAM targets. | Owner-approved General Replace safety envelopes, TP-touching Combiner refresh wiring, overlap/alignment policy, and golden outputs for released IC/modes. |
...
- NT51950/NT51951 DP Replace: exact base firmware at one owner-approved size (`0x40000`, `0x80000`, or `0x100000`), replacement DP payloads for the selected base size, expected output hashes, and rejection cases for invalid base/replacement sizes.
...
- General Replace: protected ranges beyond the current header/customer/project/backup gate, allowed explicit-mapping envelope, overlap/alignment rules, and any TP-touching post-processing triggers.
