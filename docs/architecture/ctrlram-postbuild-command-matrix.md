...
- NT51931 official BAT shape uses `NT51930BASED_NORMAL_MODE CRC8`. Registered Combiner 1.13.0 access-violates before output mutation. The final AUTO_PRJ-158 expected-derived control run with hash-only, self-reported 1.2.0.4 has zero replacement-payload drift and 108 bytes in eight classified header/header-copy CRC ranges. That executable is not committed, registered, or approved for redistribution, so DP/CtrlRAM/General Replace remain Not Supported while the command catalog is retained as evidence.
