...
| `1.1.11` | **Verifier orchestration reliability**: reproduce and repair the `test_verify_orchestration` failures observed in the concurrent `1.1.10` all-lane run; isolated reruns passed, and the cause is not yet known. Preserve independent scratch directories, managed-descendant cancellation and complete mixed-framework execution; identify the actual failure cause before changing timeout, retry or pass criteria. Pass the affected tests and the applicable full verifier at the fixed source. This is a follow-up allocation, not retrospective approval of the failed `1.1.10` run. |
