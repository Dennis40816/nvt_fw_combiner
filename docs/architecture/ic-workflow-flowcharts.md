...
| NT51931 | `SM-GENFLASH-V2`: packaged canonical V2 bundle is selected by Bootstrap UI/CLI through its content-hash anchor. | Not Supported. | Not Supported; the retained NT51930-based command catalog is evidence only. | Not Supported. | Exact AUTO_PRJ-158 expected-derived control has payload 0 diff and 108 classified header/header-copy CRC bytes. Hash-only diagnostic tool self-reports 1.2.0.4; registered 1.13 access-violates on official argv. Standard Merge and General Merge remain exposed; no Replace route is promoted. |
