...
| NT51926 | `SM-GENFLASH-V2`: packaged canonical V2 bundle is selected by Bootstrap UI/CLI through its content-hash anchor. | `R-DP-GENERIC`: DP profile wiring pending. | `R-CTRLRAM-LEGACY-NORMAL`: `CRC_Enable`; the non-routed V2 candidate selects exact `0x3C000` TP-work or `0x40000` full-Flash maps by reference length. | `R-GENERAL-POSTBUILD`: explicit mappings use protected-range gates; TP/CtrlRAM mappings run selected postbuild when available. | V2 Standard Merge preserves legacy golden bytes. The CtrlRAM candidate reads only `[0x00000, 0x3C000)` and preserves the full-Flash tail; direct golden parity and firmware-owner review remain required. |
