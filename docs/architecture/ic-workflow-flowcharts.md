...
| NT51931 | `SM-GENFLASH-V2`: packaged canonical V2 bundle is selected by Bootstrap UI/CLI through its content-hash anchor. | Not Supported. | Pending exact V2 route; selected postbuild is registered 1.13.0 `NT51931BASED_NORMAL_MODE CRC8`. | Not Supported. | Exact AUTO_PRJ-158 expected-derived control has payload 0 diff and 108 classified header/header-copy CRC bytes. Registered 1.13.0/51931-based is byte-identical to the owner 1.2.0.4/51930-based control; the latter remains evidence-only. Standard Merge and General Merge remain exposed until the CtrlRAM V2 route is reviewed. |
