...
日期：2026-09-23。狀態：**已排入 1.2.1；詳細設計與待決契約尚未全部批准**。
依據 `73b3d6e5d` 的唯讀檢視，沒有執行新測試、部署或讀取憑證。
[roadmap](nfc_roadmap.md) 擁有版本分配；下次 release 只含原 `1.2.0` 前的
修復／變更，本方案不加入該次 release。接續順序與待決項見
[1.2.1 handoff](v1.2.1-handoff.md#launcher-更新計畫的交接)。完整 Launcher 發布仍為
`2.0.0`；設計批准後才分批回寫既有 SPEC／ADR／contracts。
