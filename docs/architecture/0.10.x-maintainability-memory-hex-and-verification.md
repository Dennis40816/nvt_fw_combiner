...
The palette and primary-fill rule are accepted. The projector derives a typed
`MemoryContentRole` from canonical artifact/part/physical-region facts and
retains `CustomerInformation` and `Unmapped` instead of collapsing them into a
generic role. Composition coverage combines that typed destination role with
the typed compiled source: protected customer information or an established
reservation stays neutral, while a writable interval uses its DP/TP/TPB/LDC or
General writer hue. Presentation must not infer either dimension from an IC,
address literal, label, or range. Workflow disposition, bank or endpoint
identity, diagnostic severity, observed change, selection, and focus remain
separate typed dimensions rendered through patterns, boundaries, markers,
icons, and text. Profile data may select semantic firmware roles but cannot
choose product colors. This lets a future theme change update one token map
without changing firmware definitions, compiled plans, or report truth.

Protection intentionally has higher visual precedence than writer hue. A
`CustomerInformation` interval stays on the neutral Reserved role even when a
typed DP source supplies its final bytes; the visible source/detail text must
still name that writer and the excluded overlay or restore. In the current
NT51950/NT51951 DP Perspective contract, `[0x37000, 0x38000)` is therefore
neutral Reserved in both Standard Merge and DP Replace: Standard Merge receives
those bytes from DP while TP overlay excludes the interval, and DP Replace
receives them from the DP replacement while the base-TP restore excludes it.
This is presentation precedence only and does not alter any operation or byte.
...
| Reserved / protected or base-neutral structure | slate `#64748B` with `#CBD5E1` boundary | Established reservations, protected customer information, and unwritten/unmapped physical structure; the visible row states which case applies |
...
The supporting list projects one logical input/source once across the whole
panel, even when that source crosses Common/Master/Slave groups; planner
fragments never become duplicate peer headings. Each compact child range names
its typed IC group so aggregation does not hide the destination. Adjacent
fragments from that source and the same group/subtype coalesce into one
inclusive display range even when one part is replaced and the remainder is
kept, in which case the row says `Partially replaced` and explains both
dispositions. If the same source supplies two or more non-contiguous or
semantically distinct destinations, its single heading owns a compact ordered
range list. The proportional bar remains uncoalesced so its exact typed
write/keep boundaries are not hidden.
...
replay-coverage honesty, difference-range ordering, acceptance/review state,
...
Each persisted difference retains its complete before/output bytes plus two
aligned 16-byte context rows on each side, clipped to output bounds. The host
materializes only 12 to 28 rows and permits scrolling only within the selected
semantic range. A historical report must never reread a source BIN to
fabricate missing comparison bytes; a legacy report without complete replay
bytes exposes an unavailable preview.

The Report Diff Changed-ranges host uses one inline accordion. Collapsed cards
remain compact; the selected card expands in place to show Why and Result only.
Before/output byte lists are omitted from cards because the shared viewport is
the sole comparison surface. Selecting a card positions the viewport at the
range start and only that range's changed bytes receive change decoration.

The BIN Inspector adapter receives exact resolved metadata-structure instances,
Application-formatted facts, and immutable bytes for those instances. It uses
the same viewport snapshot/intent seam without a comparison plane or any edit,
save, public address-jump, whole-document-scroll, or search authority. It does
not accept an IC id or recover map/topology/slot context from byte content.
...
- Before #191, `HexEditorViewportControl` used one custom-drawn visual rather
  than hundreds of child controls but bound directly to
  `HexEditorWorkspaceViewModel`. #191 replaces it with the snapshot-only
  `HexViewportControl` and an editor-host adapter.
...
- `HexViewportIsReadOnlyAndReportAdoptionRemainsDeferred` now proves that the
  shared renderer has no editor/Application authority while #192 still owns
  Report Diff adoption.
