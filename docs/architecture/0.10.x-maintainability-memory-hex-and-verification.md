...
CRC, Header, and Header Copy are deliberately absent from Memory Layout
coloring, markers, effect rails, and legends. They are not user-selected
CtrlRAM destinations and are not the task's visual focus. Their typed
processor/write-range authority, execution result, and byte evidence remain in
the technical report and safety verification; only a warning or failure is
promoted into the operator-facing issue hierarchy.
...
For NT51950 with DP selected and TP absent, the UI retains the DP filename and
size and draws every already resolved physical-map range. A topology-dependent
