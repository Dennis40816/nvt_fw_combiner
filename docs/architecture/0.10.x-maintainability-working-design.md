...
| Slot behavior | Similar cards repeat file state, inspection projection, validation, icons, and formatter decisions. | One typed slot capability plus one shared visual control; workflows declare only real differences. |
...
| Workbench facade | About 40 Bootstrap DTOs expose catalog, inspection, display, compile, execution, and report responsibilities through one broad surface. | Migrate vertical capabilities behind focused interfaces; delete the facade only after all UI/CLI callers move. |
...
One artifact's metadata may require a field decoded from another artifact.
That prerequisite is part of the IC/map/metadata declaration, not an implicit
UI file-selection order and not a nullable parameter whose reason is lost.

The current NT51950 inspection path demonstrates the required model: the DP CMI
location differs by single/cascade context, and the current reader uses
`ChipNumber` decoded from TP NVT Backup FWConfig to select it. Existing trusted
declarations and tests remain authoritative for that firmware fact; this
working design records only how the dependency must flow through the system.
...
definition, so `WaitingForArtifact(tp-input)` can display **Load TP first**.
The phrase is localized view policy; the dependency is declared by the trusted
profile/Domain model and its current state is projected by Application. No
ViewModel hard-codes an IC id, TP rule, or CMI offset.
...
TP-derived chip number into DP CMI decoding, a failed decode becomes nullable
`CmiDpCode`, and Presentation turns two missing DP values into the generic
string `Pending`. M2 must characterize that behavior, then replace the nullable
collapse with the typed prerequisite result.
...
- resolved context/profile fingerprint and address-space identity;
- capacity and canonical physical regions;
- exact/pending/blocked projection state and stable issues;
...
valid. Exact output coverage is shown only after compilation succeeds. A
pending input length or blocked capability carries its typed prerequisite and
next action. The UI may show the known physical-map skeleton but must not
invent final byte ownership. For example, a TP-dependent DP evaluation shows
**Load TP first**, not an unexplained `Pending`; an already-loaded but invalid
TP shows its typed validation problem instead of asking the user to load it
again.
...
6. whether pending/blocked memory layout displays only the resolved physical
   map or also a clearly non-final capability envelope; and
7. the exact vertical-slice order inside `0.10.2`.
