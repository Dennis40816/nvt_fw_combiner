# ADR 0012: CMI DP Metadata and NVT-Copy FWConfig Selection
...
FWConfig is copied to the common driver-readable location ending at the NVT End Flag. The copy starts at the terminal `T` byte of `00 4E 56 54` minus `0xFFF`. FWConfig `ChipNumber` is at copy offset `0x017`.
...
| NT51950 | `0x3B016` / `0x05016` | 1IC / cascade; select from TP NVT-copy `ChipNumber` |
...
FWConfig display and TP-driven CMI selection read the NVT copy. When a flash-map absolute FWConfig start is also known, every exposed common field must match the NVT copy. Any mismatch yields no metadata, preventing a stale or corrupted primary/copy block from selecting a CMI branch.
...
- NT51950 1IC golden reads ChipNumber from TP NVT copy and resolves `0x3B016`; unit coverage verifies cascade resolution to `0x05016` without a missing-chip fallback.
- All current Standard Merge golden outputs compare every exposed FWConfig field between the absolute address and the NVT copy.
