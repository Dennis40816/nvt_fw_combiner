...
| --- | --- | --- | --- | --- |
| `BuiltInStandardMergeProfiles.DpPerspective` | None after V2 selector, summary, CLI, workbench, display, direct plan/byte assertions, and owner golden routes converge on deployed NT51950/NT51951 V2 profiles | Three-capacity direct V2 byte contract, two owner golden cases, stable unsupported-length diagnostics, and catalog/CLI/workbench regression | `v0.9.5` | retire in `refactor(standard-merge): retire DP Perspective legacy profiles` |
| `BuiltInStandardMergeProfiles.GenFlash` | Standard Merge plan/contract parity tests for the remaining families | Direct V2 Standard Merge golden coverage independent of the C# profile oracle for every retained family | `v0.9.5` | retain as evidence |
