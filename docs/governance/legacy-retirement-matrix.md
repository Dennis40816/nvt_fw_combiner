...
| Dynamic General Merge C# profile construction | None. Default General Merge, CLI, and UI route through the registered logical-output V2 profiles for all built-in ICs; persisted saved rules retain only a profile-id compatibility alias | Synthetic full-output legacy/V2 differential for every then-admitted IC, retained as a hand-constructed mapping oracle after retirement; default-route byte/report/UI/saved-rule regression; no direct firmware Golden, physical-map, processor, CRC, or firmware-support claim | `v0.9.5` | retired in `a9a65b49` (`refactor(general-merge): route default workflow through V2`) |
...
| Legacy compiled-composition identity, compilation authority, and runtime-executable admission | None. Preview, Build, Report, input classification, and capability binding admit only the typed V2 compiled composition | Exact fingerprint/report/golden parity; Domain/Application/Bootstrap tests; architecture exclusion of legacy creators and eligibility | `0.10.x` | retired by issue #196; `LegacyCompiledCompositionIdentity`, `CompositionCompilationAuthority`, legacy factories, and alternate eligibility branches are physically removed without byte or support change |
| Per-IC C# built-in bundle and workflow registration literals | None. Build, runtime, capability inventory, General Replace Saved Rule routing, support projection, and release packaging consume `profiles/built-in/package-trust-index.json` | Strict schema/hash/version validation, route uniqueness, unchanged compilation fingerprints/reports, release closed-allowlist dry run | `0.10.x` | retired by issue #196; generic Bootstrap projections remain adapters over immutable package data, not another authority |
