...
self-contained composite ReadyToRun single-file `win-x64` Avalonia app with
trimming disabled, copies
...
or change the closed package contents. Composite ReadyToRun precompiles managed
methods to reduce cold JIT cost; it does not authorize trimming, a separate
runtime dependency, or any firmware/profile semantic change.
