...
- Owner-approved golden firmware fixtures use role `goldenFixture`; non-BIN reference evidence uses role `reference`. `goldenFixture` is a manifest storage role only: it does not classify a Direct Golden, input-only evidence, parity, support, or runtime semantics.
- The v1.1.2 stable snapshot includes only the exact hash-pinned `release-canonical-v1.json` schema `1.1` selection: 25 Direct Goldens, one owner-certified NT51929 input-only evidence case, and nine aliases. Its two neutral-named raw BIN observations are ordinary outer-ZIP entries, individually hashed in this manifest, SBOM, provenance, and checksum inventory; they are never nested ZIP/7z content or a raw-file bypass.
