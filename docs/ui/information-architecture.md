...
- Replace UI requires IC num selection/input before showing profile-specific regions. First UI supports `single` and `cascade`; `numeric` remains reserved for future IC exceptions.
