...
- Home: the first screen has three primary cards for Settings, Replace, and Merge plus a separate Hex Editor workspace entry.
...
- Workflow device context: IC and Number live in one fixed context area below the header on Merge, Replace, and Hex Editor pages. IC is the target IC family/model selector; Number is the IC count/variant selector such as `single`/`cascade` or profile-specific numeric choices. Home and Settings hide this context because they do not author firmware operations.
...
| Hex Editor | Experimental binary patch workspace | shared IC/Number context, base BIN slot, profile-authorized hex viewport, virtual diff, patch audit, and Build | General Replace mapping table, direct file writes, insert/delete, range authorization logic in UI |
...
Shell
  ...
  Context bar: breadcrumb history, workflow IC/Number only on Merge, Replace, and Hex Editor
  Content region
    Home launcher: Settings | Replace | Merge cards plus Hex Editor entry
    Settings page
    Merge page
    Replace page
    Hex Editor page
...
- IC and Number remain in the same fixed Device context location across Merge, Replace, and Hex Editor, and are hidden on Home and Settings.
...
- Home uses three primary launcher cards: Settings, Replace, and Merge, plus a separate Hex Editor entry.
...
- IC and Number use a shared workflow Device context row that stays in the same location on Merge, Replace, and Hex Editor pages.
