...
### Desay rule apply lifecycle and editor proposal — 2026-09-14

The owner accepts the proposed lifecycle, while noting that ordinary Settings
currently applies immediately and asking whether this editing needs a separate
modal page:
...
Recommended presentation, pending owner approval/preview: a **Desay rules**
entry with current ID summary and an **Edit rules** action opens a dedicated
transactional editor within the existing modal host. Settings is already a
modal, so replace/suspend its content and restore the same section on return
rather than stack independent modal windows or lose focus/navigation state.
Keep edits local until **Save and apply**; **Cancel** leaves effective rules
unchanged. Use existing shared modal/button/validation styles. Restore defaults
in the editor prepares a draft, with the same explicit save boundary; dirty
close/Escape should reuse the existing discard-confirmation convention.
...
pattern, not its version-management service or firmware authority. No new
visual reference is approved and no production UI/configuration is implemented
by this discussion record.
