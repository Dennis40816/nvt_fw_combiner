...
Current allocation (owner resequencing, updated 2026-09-16) is owned by the
...
Settings conveniences and Toolchain Runtime selection in `1.1.9`, and broader
per-page option density in `1.2.4`. This replaces the preceding blanket `1.1.7` UI allocation. General
and maintainer rule-authoring remain separate `1.3.x` work.
The gate/agent workflow review moves to `1.1.8`. Tentative `1.1.10` adds
CtrlRAM Replace with AB Code Flash inputs for NT51932 Perfect family and
NT51950 Partial family. This is a queued support target, not a current support
claim: bank targets, preserved ranges and coupled integrity behavior need
explicit contracts and independent evidence. The roadmap owns its schedule;
this work does not expand the current `1.1.7` UI closure.
