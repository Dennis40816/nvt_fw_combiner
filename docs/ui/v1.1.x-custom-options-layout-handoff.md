...
Run `dotnet test tests/<project>/<project>.csproj --no-restore --filter '<filter>'`; latest scoped results: Application **133 passed** (`ActiveSessionBuildBlocker|AuthoringInputSlotInspection|CapabilityActionReadiness|EventBufferFormat|AuthoringSession`, each prefixed `FullyQualifiedName~`); Bootstrap **113 passed** (`EventBufferFormatConfigurationHostTests|AbMergeFormatRuntimeTests|AbMergeCliCommandTests`); UI **50 passed** (`EventBufferFormat|BuildBlocker|MessageCenter`); Architecture **13 passed** (`ProjectDependencyTests|ApplicationConvergence|AuthoringConvenienceFacades|CompositionProgressPresentation`). Bootstrap includes the final stale-return correction; Application/UI evidence precedes only that bounded correction.
Full Architecture run at defaults unit was **254 passed / 2 failed**: inherited `ShellUsesCleanHomeAndIndependentWorkflowPages` old ScrollViewer assertion, and this handoff's former 2566-line ceiling violation. Condensing this checkpoint to 2495 lines closed the ceiling failure; its targeted Architecture recheck passed 1/1. The old ScrollViewer assertion remains an integration blocker. Neither failure was introduced by the settings commits.
Logs remain under `D:\NvtFwCombiner-TestArea\config110-*.log`; `git diff --check` passed. No fresh full-suite or release Golden claim. `CONFIG-DEFAULTS-110-01` and `CONFIG-ERRORS-110-01` are design-active local records, not finalized integration evidence; checkpoint-wide ownership/evidence finalization and required CI remain outstanding. Latest `python scripts/verify.py --structure-only` failed with four integration-record errors: the two committed design-active records, plus uncovered prior `MainWindow.axaml` and `Views/FirmwareSlotCard.axaml` changes. JSON working-copy LF now matches index bytes. No push, merge or release performed.
