...
is approved and locally implemented/verified below. Next close and consolidate
the remaining Desay locator, bank/write-range, evidence and configuration
lifecycle decisions for the urgent `1.1.6` slice. Editable Settings is now part
...
`0x22200`, with this field at `0x2220C`. The numerical discrepancy is closed;
primary-versus-Backup authority for the new detector remains to be decided.
...
`0x97` and `0xA6` are already declared matches for that format. Primary-versus-
Backup authority, any proposed manual override and the remaining Settings
...
| --- | --- | --- |
| A — detection and Info | Read one byte at FWConfig-relative `+0x0C`; either `0xA6` or `0x97` matches Desay (OR comparison, not a two-byte signature). Owner corrects the primary field address to `0x2220C`, with base `0x22200`; `0x2200C` was a typo. Show the actual value as plain text, e.g. `0x97 - Desay`. A valid nonmatch selects Common; missing/invalid required FWConfig blocks rather than defaulting to Common. TPA/TPB Common-versus-Desay disagreement also blocks. Successful detection automatically applies Desay gating/layout in scope. | Decide whether the new detector uses the primary field or the dynamically located Backup already used by AB admission. The numerical correction is not a reader-selection decision. Ambiguous rule configurations still need explicit outcomes. Automatic activation does not authorize an unverified map or broaden applicability. |
...
- Current public/candidate NT51950 and NT51951 profiles declare FWConfig start
  ...
  `0xFFF` as the Backup start. Proposed for owner decision: use this existing
  validated Backup plus `0x0C` for Desay detection as well, avoiding a second
  conflicting runtime source; do not silently fall back to the primary.
  This proposal is not implemented or approved by the typo correction.
