...
| Config page | Add a large **Config** section to the existing Settings modal, no second editor modal. Under the vendor-neutral Event Buffer subject, separate **Display names** (text only) from **Format detection** (one editable ID group per admitted format, read-only scope/output effects). No per-alias Output rule editor or vertical table gridlines. Keep ordinary Preferences immediate-apply. |
...
Current visual review: [task-separated Config preview](#task-separated-config-preview--2026-09-14).
The owner accepted separating name lookup from grouped format recognition after
rejecting A/B as difficult to use. Group navigation by configuration subject,
not vendor. The exact replacement preview remains pending visual approval;
the earlier stacked, vendor-grouped and A/B images are iteration history.
...
now settled; do not re-ask them. Review the Config preview, then consolidate
