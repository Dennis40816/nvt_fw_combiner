...
is the only candidate mechanism. Detection versus automatic layout activation,
manual override and ambiguous matches still require a consolidated decision;
...
future expansion requires explicit applicability. Expected capacity is now
stated as **8 MB**, superseding the earlier literal `8mib`; exact byte count
remains to be settled. The owner confirms the size check measures the **DP AB
...
This records requested behavior, not approval of auto-activation, override,
warning-to-block escalation, exact firmware locators or a settings editor.
...
| --- | --- | --- |
| A — detection and Info | Read FWConfig at relative `0x0C`; owner estimates absolute address `0x2200C`. Values supplied are `0xA6` and `0x97`, indicating a Desay case. The accepted badge wording includes the actual matched value: `Desay Format 0x97`, rather than only generic detection text. | Confirm exact artifact/address space, variant FWConfig base, field width and match operator: one byte matching either value versus an ordered two-byte signature. Do not silently change `0x2200C` to the current public address. Missing/truncated input, no match, conflicting inputs and ambiguous rules need explicit outcomes. Detection alone does not yet decide automatic map activation. |
...
| B — capacity warning | Check the **DP AB Code input file** against expected **8 MB**; a mismatch should raise a **warning**. This supersedes the initial `8mib` wording and resolves which artifact is measured. | Exact byte count remains unresolved: decimal 8 MB is 8,000,000 bytes; 8 MiB is 8,388,608 bytes (`0x800000`). Do not apply this size check to TP files or final output, or silently escalate the warning to a Build blocker; existing independent range/safety failures still block as required. |
| C — TP Backup | For the declared NT51950/NT51951 partial-family Desay case, TP Backup start must become `0x4A000`. | Reuse the explicitly shared fact under Desay applicability, not a whole-map Perfect-family override. Confirm member list, variant/revision, workflows, topology, source length/end and address space; especially distinguish Standard backup from AB TP B relocation and CtrlRAM behavior. Public/non-Desay layouts must not inherit this change. |
...
| E — public backup update | Public NT51950/NT51951 TP Backup may also have changed; discuss later. | Keep this as a separate unresolved public-map revision. No replacement address is supplied and the public map must not be set to `0x4A000` merely because Desay uses it. Obtain the public revision/source evidence first. |
...
acknowledgement checkboxes and automatic Desay activation are not approved.
