...
| Config page | Add a large **Config** section to the existing Settings modal, no second editor modal. Initially show Desay detection and separate Event Buffer ID/name lookup, without vertical table gridlines. Keep ordinary Preferences immediate-apply. |
...
Current visual review: [vendor-neutral alternatives A/B](#vendor-neutral-config-alternatives--2026-09-14).
The owner requests a denser, categorized interface but rejects the previous
vendor-specific Desay navigation. Group by configuration subject, not by vendor;
the earlier stacked and vendor-grouped images are iteration history.
