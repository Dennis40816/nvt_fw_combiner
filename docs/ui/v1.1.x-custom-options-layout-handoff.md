...
Start-of-development audit, 2026-09-14: no active goal exists. One concrete
firmware write-authority question must be resolved before establishing the
whole `1.1.6` implementation goal: the Desay workbook TP envelope is
`[0xA000,0x3F000)`, while the current AB TP template is
`[0xA000,0x37000)` (start 40960, length 184320). The additional range includes
the currently identified Customer Information interval `[0x37000,0x38000)`.
Confirm whether the expanded envelope is writable TP content or allocation
only, and whether Customer Information must remain from the DP seed. The
already accepted B start `0x4A000` does not settle TP copy length. See the
...
Do not guess a wider write range or alter Golden expectations. Further coupled
offset/processor checks are engineering work against the resolved contract,
not a request to repeat the settled Config decisions. No production code or
firmware behavior changed in this audit.
