...
**`Event Buffer Format`**, retaining the actual value, e.g. `Desay · 0x97`
...
- **TP Version comes first. PID is primary information and more important than
  IC Count.** Proposed default four-field order: TP Version, PID, Common FW
  Version, Event Buffer Format; put IC Count among secondary Details fields.
  The latter exact placement is a scheduling/design recommendation, not a new
  firmware interpretation. PID must use its existing canonical metadata
  binding/formatter; do not substitute Jira Index or infer it from a filename.
- Use the exact label **Event Buffer Format**, plain text such as
