...
metadata warnings. Manual overrides, exact firmware locators, cross-input
format conflicts and the Settings editor details remain unresolved; the output
...
| --- | --- | --- |
| A — detection and Info | Read one byte at FWConfig-relative `+0x0C`; either `0xA6` or `0x97` matches Desay (owner-confirmed OR comparison, not a two-byte signature). Owner estimates absolute address `0x2200C`. Show the actual value for both formats, e.g. `Desay Format 0x97` or `Common Format 0x84`. A valid nonmatch selects Common; missing/invalid required FWConfig blocks rather than defaulting to Common. Successful Desay detection automatically applies the applicable Desay gating and layout in the approved scope. | Confirm exact artifact/address space and variant FWConfig base. Do not silently change `0x2200C` to the current public address. Conflicting inputs and ambiguous rules still need explicit outcomes. Automatic activation does not authorize an unverified map or silently broaden applicability. |
