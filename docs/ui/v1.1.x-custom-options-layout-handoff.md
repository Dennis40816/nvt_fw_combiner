...
CtrlRAM AB Replace support remains unimplemented. Use the [existing intake](#ctrlram-replace-ab-code-intake--2026-09-16); owner decisions/evidence remain for A/B/both/default target, shared/per-bank sources, 932 relocation, 950/951 coupled header/CRC order, exact writable/preserved ranges and independent outputs. Do not infer firmware rules from these configuration fixes or from the old non-certified D86 demo.
...
Open 1.1.10 decisions: A-only/B-only/both and default bank, shared/per-bank
sources, already-relocated B addresses, coupled header/CRC ordering and exact
allowed writes. `testdata/golden/canonical/manifest.json` contains existing
CtrlRAM and AB Merge evidence, not CtrlRAM AB Replace full-output certification.
