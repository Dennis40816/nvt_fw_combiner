...
| Config page | Add **Config > Event Buffer Format** inside the existing Settings modal. Combine each owner-defined unique vendor identity and its user-editable alias in one entry; the fixed identity, never the alias, binds supported output effects. Ordinary users cannot add identities or edit effects. Recognition-byte editability needs confirmation against the earlier editable-ID request. No generic Output rule editor or vertical table gridlines. Keep ordinary Preferences immediate-apply. |
...
a fixed vendor identity and editable alias. The earlier task-separated image,
...
- Fixed vendor identity: owner-defined, bound to admitted profile behavior;
  ordinary Settings edits cannot create or rebind it. Exact serialized key
