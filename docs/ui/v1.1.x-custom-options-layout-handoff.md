...
Code input file**, not TP inputs or final output. The detection badge must show
the matching value, e.g. **`Desay Format 0x97`** (or `Desay Format 0xA6` when
that value matches), and Output must identify IC, Mode and vendor/public baseline.
...
Owner follow-up also requests the actual value for public inputs, e.g.
**`Common Format 0x84`**. `0x84` is a display example, not a newly declared
...
Detection badge and effective output format are distinct facts; pending or
...
Information presentation proposal, 2026-09-14 (not a visual approval): keep a
single compact Format row/badge in the primary Information summary, showing
`Common Format 0x84` or `Desay Format 0x97` from the actual accepted input.
Use the existing badge/component styling; Common is neutral, Desay is an
informational accent, and invalid input uses the existing Error treatment.
Do not represent a vendor format as a warning merely because it is customized.
Keep the primary field set bounded and aligned in the existing responsive
grid; place future secondary metadata in a collapsed Details area, grouped
by meaning rather than adding an ever-wider row of fields or badges. Field
labels stay aligned, values wrap or disclose full text accessibly. Blocking
issues stay visible without hover or expansion. Detection source/address and
rule revision belong in Details/Report; the effective IC/Mode/Format must also
remain visible in output confirmation. Reuse shared Information rendering
across pages; actual viewport/reference review is still required before UI
implementation.
