...
stated as **8 MB**, superseding the earlier literal `8mib` below; exact byte
count and measured artifact remain to be settled. Use a badge for detected
Desay format, and make Output identify its IC, Mode and vendor/public baseline.
...
| B — capacity warning | Desay is expected to use only `8mib`; a mismatch should raise a **warning**. | Preserve the owner's literal unit until resolved: `8 Mib` is 1,048,576 bytes, whereas `8 MiB` is 8,388,608 bytes. Confirm whether the rule measures DP/full-image file size, declared flash capacity or output size. Do not silently make this new mismatch warning a Build blocker; existing independent range/safety failures still block as required. |
