...
| --- | --- | --- |
| A — detection and Info | Read one byte at FWConfig-relative `+0x0C`; either `0xA6` or `0x97` matches Desay (owner-confirmed OR comparison, not a two-byte signature). Owner estimates absolute address `0x2200C`. The accepted badge wording includes the actual matched value: `Desay Format 0x97`, rather than only generic detection text. Successful detection automatically applies the applicable Desay gating and layout in the approved scope. | Confirm exact artifact/address space and variant FWConfig base. Do not silently change `0x2200C` to the current public address. Missing/truncated input, no match, conflicting inputs and ambiguous rules need explicit outcomes. Automatic activation does not authorize an unverified map or silently broaden applicability. |
