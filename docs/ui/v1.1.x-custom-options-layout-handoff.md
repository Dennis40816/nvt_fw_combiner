...
Overrides, warning-to-block escalation, exact firmware locators and a Settings
editor remain unresolved; the output confirmation presentation is still open.
...
| --- | --- | --- |
| A — detection and Info | Read FWConfig at relative `0x0C`; owner estimates absolute address `0x2200C`. Values supplied are `0xA6` and `0x97`, indicating a Desay case. The accepted badge wording includes the actual matched value: `Desay Format 0x97`, rather than only generic detection text. Successful detection automatically applies the applicable Desay gating and layout in the approved scope. | Confirm exact artifact/address space, variant FWConfig base, field width and match operator: one byte matching either value versus an ordered two-byte signature. Do not silently change `0x2200C` to the current public address. Missing/truncated input, no match, conflicting inputs and ambiguous rules need explicit outcomes. Automatic activation does not authorize an unverified map or silently broaden applicability. |
