...
Owner-requested restart point; this dated snapshot is not a new contract,
release approval or competing roadmap. Verify Git status before continuation.
Checkout: `codex/1.1.10-mode-scroll-layout`, based on released 1.1.9
`b56b4eee5914f1651ed40747ea0ba48015a52105`. Last completed commit before this
handoff: `ded0ec64`. Continue in this checkout; unfinished work is not in commits.

### Completed local units

- `0edd6c11` / `c732bcf4`: stable Merge Mode anchor and matching tests;
  reserve the shared vertical scrollbar gutter.
- `b45965b8`: remove empty firmware-slot bottom whitespace without changing
  loaded-card action alignment. Prior scoped UI run: 256 passing tests.
- `ded0ec64`: [local user-data inventory](../architecture/local-user-data-inventory.md)
  for future uninstall. No deletion behavior implemented.

### First continuation: configuration defaults and error clarity

Owner approved fixing clean-start AB Merge blocked by absent
`%LOCALAPPDATA%\NvtFwCombiner\event-buffer-format.v1.json`, with regression
tests and clear configuration-specific errors. Missing custom file should use
built-in defaults without saving; invalid/unreadable custom configuration still
blocks. Draft edits apply only after Save. Downloads do not clear user settings.

Uncommitted R2 implementation changes `EventBufferFormatConfigurationSession`
and its `.State.cs`, Settings EventBufferFormat ViewModel/resources, ADR 0072,
the configuration contract, Application session tests, Bootstrap
`AbMergeFormatRuntimeTests`, and UI `ShellViewModelTests.EventBufferFormat`.
`CONFIG-DEFAULTS-110-01.json` is staged design admission, not final evidence.
Preserve all these changes; inspect the exact diff instead of starting over.

Current design publishes Ready with `UsesBuiltInDefaults`, a deterministic
domain-separated hash, and no file write; it preserves LastSaved provenance.
Independent reviewer `config_defaults_review` identified remaining checks:

1. Prove an actual first-use Build without Save; the new Bootstrap test only
   prepares inputs/proposal. Do not weaken Golden expectations.
2. Save a genuinely different custom configuration before deletion; prove
   default hash/flags and editor baseline/Discard recover defaults, not stale
   LastSaved. Review `IEventBufferFormatConfigurationSession` documentation.
3. Reconcile old Missing expectations in Bootstrap
   `EventBufferFormatConfigurationHostTests` and
   `AbMergeCliCommandTests.FormatAdmission`; retain genuine stale-run rejection.
4. Latest completed command:
   `dotnet test tests/NvtFwCombiner.UiSmoke.Tests/NvtFwCombiner.UiSmoke.Tests.csproj --no-restore --filter FullyQualifiedName~EventBufferFormat`
   returned **35 passed, 1 failed, 0 skipped** on this unfinished tree.
   Failure: `EventBufferFormatMountedBeforeFirstLoadEnablesSaveAndRestore`,
   `XamlControlStyleContractTests.EventBufferFormat.cs:306`, expected True,
   actual False. Inspect intended first-load behavior before changing assertion.
   No full-suite pass or integration-ready claim exists for this patch.

Error-clarity fix remains unimplemented. Trace existing owners:
`AbMergeAuthoringExperience.InputInspection` -> inspected status/readiness ->
`ActiveSessionBuildBlockerResolver` -> Merge presentation. Configuration errors
currently become repeated slot InputBlocked/Error results. Preserve captured
sources and fail-closed Build admission; render one actionable configuration
prerequisite instead of blaming every BIN. Do not empty the status dictionary:
`BuiltInFirmwareInspection` indexes it. Also cover configuration reapply in
`MergePresentationViewModel.AbMergeAuthoring`. Extend/admit exact mutable paths
before additional governed behavior changes, then scoped review and coherent
commits. An old demo used CtrlRAM D86 inputs, not certified AB Golden inputs;
the defaults fix does not establish those files as valid AB inputs.

### Then: CtrlRAM Replace of AB containers

Use the [existing intake below](#ctrlram-replace-ab-code-intake--2026-09-16).
Support is not implemented. Owner decisions/evidence remain for target bank
A/B/both and default, shared/per-bank sources, 932 relocation, 950/951 coupled
header/CRC ordering, exact preserved/writable bytes and independent outputs.
Memory Layout/readiness/Report must disclose admitted behavior. The roadmap
retains completion before 1.2.0; this checkpoint adds no firmware decisions.

### Efficient continuation notes

Use this checkpoint plus `git status`/scoped diff for progress questions. Read
older sections or other owners when a dependency or conflict requires them;
required skills/instructions still need their complete prescribed reads.
For code work, use the existing CodeGraph entry point, then named symbols and
bounded source excerpts. Keep full test logs in the external test area and
return counts plus failing cases, expanding only the relevant failure.
Reuse review findings for unchanged scope/source; delegate bounded questions
with relevant context rather than the conversation history. On the next
handoff, update this checkpoint's state/evidence instead of appending another
execution diary. These are applications of existing scope/review rules, not
token caps or waivers of tests, firmware evidence or release gates.

Before tests, set TEMP/TMP/TMPDIR to the existing `temp` child of user-level
`NFC_TEST_AREA_ROOT` (currently `D:\NvtFwCombiner-TestArea`) in that process,
as required by root instructions. New conversations must not assume old tool
session IDs or reviewer instances remain callable.
