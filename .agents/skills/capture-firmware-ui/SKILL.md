...
These renders use real product XAML and manifest-backed input files in AvaloniaHeadless. For OS window fidelity, launch the corresponding Desktop case with `$open-firmware-example` and inspect it there. The images are presentation evidence only. Both AB CtrlRAM cross-case examples remain Candidate. `nt51950-ab-ctrlram-unsupported` is a historical case ID; report the current UI status and manifest evidence disposition instead of inferring support from that name. Screenshots do not certify output byte parity or Golden support.
