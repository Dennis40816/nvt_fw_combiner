...
A Codex skill with a trigger-rich **description** whose
`agents/openai.yaml` leaves `policy.allow_implicit_invocation` enabled or
omitted. The agent may select it autonomously and the human may still type its
name. It pays **context load** for discoverability. Pick this mode only when
autonomous reach is necessary.

_Avoid_: ability, tool, capability

### User-Invoked

A Codex skill whose `agents/openai.yaml` sets
`policy.allow_implicit_invocation: false`. It keeps the required concise
**description** for the human catalog, but only the human explicitly invoking
`$skill-name` starts the workflow. Routers may point to it without silently
executing it.
...
The required `SKILL.md` summary. For a **model-invoked** skill it is the
machine-readable trigger; for a **user-invoked** skill it is a concise
human-facing catalog summary. Invocation mode itself is controlled by
`agents/openai.yaml`.
...
The token and attention cost of skill catalog metadata and any loaded skill
body. Model-invoked descriptions pay more because they carry reliable trigger
branches; user-invoked descriptions should remain shorter.
...
when to type each name. Model invocation reduces this by making the agent an
index. Spend cognitive load where explicit human intent matters.
...
naming each and when to reach for it — so the human has one entry point. It can
recommend them but `allow_implicit_invocation: false` keeps their execution
explicit.
...
**description**, no **steps**, not invocable — that any skill can point at. It
is the shared home for material that need not trigger on its own.
