...
internal sealed partial class UiLaunchOptions
{
        ...
        string? reportPath, bool openReport, List<string> unknownArguments, List<string> issues)
    {
        ...
        if (options.ContainsKey("--base") || options.ContainsKey("--ctrlram") || options.ContainsKey("--tp"))
        {
            issues.Add("AB startup cannot be combined with CtrlRAM or Standard Merge inputs.");
        }
