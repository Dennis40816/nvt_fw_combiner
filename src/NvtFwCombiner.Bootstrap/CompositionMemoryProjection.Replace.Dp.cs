...
public static partial class CompositionMemoryProjection
{
    ...
    {
        if (!CanonicalCapabilityProjection.TryResolveBuiltInV2DpReplaceDisplay(icId, baseCapacity, out BuiltInV2DpReplaceDisplay? display))
        {
            return null;
        }
        ...
        if (display.RequestedBaseCapacity is null && display.Issues.Count == 0)
        {
            ...
            return CreateMessageDisplay(
                ...
                ("Reference length pending", "Reference FlashCode required", $"Select a complete Standard/Normal Merge FlashCode to resolve the actual DP Replace length ({capacities})."));
        }
        ...
        if (display.Composition is not { } composition)
        {
            ...
            string issues = display.Issues.Count == 0
                ? DpReplaceProfileUnavailable
                : CompositionExecutionAdapter.FormatIssues(display.Issues);
            return CreateMessageDisplay(
                failureLabel,
                ...
                unsupportedLength
                    ? ($"Unsupported {BootstrapRangeText.FormatHex(requestedCapacity)}", "Unsupported reference", $"This Reference FlashCode length is not approved for {icId} DP Replace; use {BuiltInV2Bundle.FormatCapacities(display.SupportedBaseCapacities)}.")
                    : (failureLabel, "Invalid profile", issues));
        }
    ...
    internal static bool TryGetV2DpReplaceInputDescription(string icId, out string description)
    {
        if (!CanonicalCapabilityProjection.TryResolveBuiltInV2DpReplaceDisplay(icId, baseCapacity: null, out BuiltInV2DpReplaceDisplay? display))
        {
        ...
        if (display.Issues.Count != 0 ||
            ...
            !CanonicalCapabilityProjection.TryResolveBuiltInV2DpReplaceDisplay(
                icId,
                display.SupportedBaseCapacities[0],
                out BuiltInV2DpReplaceDisplay? resolved) ||
            resolved.Composition is not { } composition)
        {
            description = $"The V2 DP Replace profile is unavailable: {(display.Issues.Count == 0 ? DpReplaceProfileUnavailable : CompositionExecutionAdapter.FormatIssues(display.Issues))}";
            return true;
        }
