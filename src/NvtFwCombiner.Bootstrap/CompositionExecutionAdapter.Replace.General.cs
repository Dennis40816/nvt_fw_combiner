...
public static partial class CompositionExecutionAdapter
{
        ...
        ActiveSessionSnapshot? acceptedSession = null)
    {
            ...
            CapabilityActionReadinessSnapshot? readiness = null)
        {
            return strategy.RenderFailure(new GeneralReplaceRunFailure(
                ...
                CanonicalAuthoringAdapter.IsAcceptedGeneralMappingDraft(context.MappingDraft)
        ...
        GeneralSelectedFileBindingResult acceptedFiles =
            CanonicalAuthoringAdapter.RequireAcceptedGeneralSelectedFiles(context!.MappingDraft);
        ...
        GeneralReplaceV2Registration? validationRegistration =
            CanonicalAuthoringAdapter.ResolveGeneralReplaceValidationRegistration(normalizedIcId);
        if (exactParentOverride is null && validationRegistration is null)
        {
            admission = CanonicalAuthoringAdapter.AdmitGeneralMappingDraft(
                context.MappingDraft,
                context.Capacity,
        ...
        if (!CompositionPlanningAdapter.TryCreateGeneralReplaceMappings(
                admission,
                context.Capacity,
                ...
                out IReadOnlyList<CompositionPlanningAdapter.GeneralReplacePatchArtifact> patchArtifacts,
                out IReadOnlyList<CompositionIssue> mappingIssues))
        {
            return Blocked(mappingIssues);
        }

        bool postbuildProfileResolved = CompositionPlanningAdapter.TryGetPostbuildProfile(
            icId,
            context.BasePath,
        ...
        LegacyCombinerPostbuildCommandPlan? commandPlan = CompositionAuthoringSessionAdapter.TryPlanGeneralReplacePostbuild(
            icId,
            context.Selection,
            context.Capacity,
        ...
        if (resolvedCapability is null && !CompositionAuthoringSessionAdapter.TryResolveGeneralReplaceCompiledRoute(
            registration,
            context.Capacity,
            context.Selection,
            context.MappingDraft,
        ...
        if (touchesTpRegion)
        {
            (CapabilityActionReadinessSnapshot readiness, string? requiredStageId) =
                await CompositionAuthoringSessionAdapter.ResolveGeneralReplacePostbuildReadinessAsync(
                    admission,
                    exactParent.Runtime,
                    resolvedCapability,
                    authoringRevision,
        ...
        if (!CompositionPlanningAdapter.TryMaterializeGeneralReplacePatchArtifacts(
                patchArtifacts,
    ...
    internal static bool GeneralReplaceTouchesTpRegion(
