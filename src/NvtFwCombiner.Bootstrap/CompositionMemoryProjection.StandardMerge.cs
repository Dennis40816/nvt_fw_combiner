...
/// <summary>Projects compiled composition plans into immutable memory-layout display models.</summary>
public static partial class CompositionMemoryProjection
{
    ...
    public static WorkbenchMemoryDisplay GetStandardMergeMemoryDisplay(string icId, long? dpInputLength)
    {
        ...
        bool lengthPending = dpInputLength is null &&
        ...
        if (lengthPending)
        {
            return CreateMessageDisplay(
                ...
                (
                    "Selected DP BIN length pending",
                    "DP length pending",
                    $"Select a DP BIN before final ownership is drawn. Supported DP lengths are {CanonicalCapabilityResolution.FormatStandardMergeSupportedDpLengths(icId)}."));
        }
        ...
        {
            string detail = CompositionExecutionAdapter.FormatIssues(issues);
    ...
    {
        return !CanonicalCapabilityResolution.TryGetBuiltInV2StandardMergeContainerPolicy(
                icId,
                out V2StandardMergeContainerPolicy? policy)
