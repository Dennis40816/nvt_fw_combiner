...
internal static class PinnedJsonCatalogLoader
{
    ...
    /// <summary>Loads one hash-pinned catalog through explicitly generated JSON metadata.</summary>
    internal static T Load<T>(
        ReadOnlySpan<byte> bytes,
        string expectedSha256,
        string catalogName,
    ...
    private static void VerifyHash(ReadOnlySpan<byte> bytes, string expectedSha256, string catalogName)
    {
