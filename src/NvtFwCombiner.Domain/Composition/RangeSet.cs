...
public sealed class RangeSet
{
    ...
    public RangeSet(IEnumerable<ByteRange> ranges)
    {
        ...
        _ranges = ranges.OrderBy(range => range.Start).ThenBy(range => range.Length).ToArray();
    }
    ...
    public bool Contains(ByteRange candidate) => _ranges.Any(range => range.Contains(candidate));
