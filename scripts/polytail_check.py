...
SKIP_PARTS = {"refcode", ".git", ".dotnet", "bin", "obj", "dist", "artifacts", "__pycache__"}
    ...
    for root in SCAN_ROOTS:
        for path in root.rglob("*"):
            ...
            if path.name != "polytail_check.py" and PLACEHOLDER_PATTERN.search(text):
                errors.append(f"placeholder marker in production/tooling source: {rel.as_posix()}")
            if path.name not in {"polytail_check.py", "validate_repository.py"}:
                for token, guidance in STALE_TOKENS.items():
                    if token in text:
                        errors.append(f"stale token {token!r} in {rel.as_posix()}: {guidance}")
            line_count = text.count("\n") + 1
            if path.suffix.lower() in {".cs", ".py"} and line_count > 800:
                errors.append(f"oversized source file ({line_count} lines): {rel.as_posix()}")
