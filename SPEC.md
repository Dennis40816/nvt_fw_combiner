...
> 文件狀態：`0.9.13 stabilization release candidate with 0.9.15 deferred scope`
> 文件版本：`0.9.13`
> 基準日期：`2026-07-22`
...
As of 2026-07-22, `0.9.13` is an urgent support-neutral stabilization release based on the exact latest `0.9.12` head. It fixes action-rail visibility/layout, primary-button hover contrast, exact-file Explorer reveal, TP-version presentation and output-name synchronization, Inputs scrolling, OneDrive/reparse diagnostics, NT51951 DP guidance, and Standard/Customized display labels. It changes no firmware range, operation order, CRC/header behavior, processor authority, profile admission, report schema, stable workflow token, or support stage. Shared Hex viewport/Changes redesign, global Button pressed feedback, and AB Code production re-admission are deferred to `v0.9.15`.
...
- FlashCode output naming uses the fixed `NT51xxx_FlashCode_DxxxxTxxxx_YYYYMMDD.bin` form and retains its catalog-owned legacy two-byte DP token for compatibility. `v0.9.13` changes only CtrlRAM edited-TP filename synchronization. The deferred AB/UI work may move displayed DP facts to the IC-owned three-byte CMI Reg16h-18h decoder whenever that layout exists, without falling back to the legacy naming pair; output-naming migration remains a separate compatibility decision. TP uses the validated FW version and FW sub-version bytes. UI never infers version bytes from file names.
