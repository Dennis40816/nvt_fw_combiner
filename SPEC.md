...
- FlashCode output naming uses the fixed `NT51xxx_FlashCode_DxxxxTxxxx_YYYYMMDD.bin` form and retains its catalog-owned legacy two-byte DP token for compatibility. DP facts shown in the UI use the IC-owned three-byte CMI Reg16h-18h decoder whenever that layout exists; they never fall back to the legacy naming pair. TP uses the validated FW version and FW sub-version bytes. UI displays decoded tokens or explicit unknown placeholders and never infers version bytes from file names. Output-naming migration to CMI remains a separate `v0.9.13` compatibility decision.
