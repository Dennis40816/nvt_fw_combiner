...
28. General Merge and General Replace use one `AuthoringMappingState`, one
    ...
    Source Slice and From File Start are presets over the same operation; From
    File Start fixes source start at zero. `Use full file length` materializes
    the accepted inspected length into the editable row and is never a dynamic
    execution mode. Each selected General file is accepted as immutable length
    plus SHA-256 for one definition and authoring revision. Explicit
    Reload/Rebind advances the revision and preserves the row's concrete range.
    Preview/Build verify reopened bytes against the immutable accepted stamp
    before composition consumes them. Path, display name, and last-write time
