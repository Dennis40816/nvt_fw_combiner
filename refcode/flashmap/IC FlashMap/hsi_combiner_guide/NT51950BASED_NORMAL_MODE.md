...
## Summary
This command performs a normal merge for NT51950-based firmware binaries and produces a complete firmware binary file. The process includes:
...
- argv[1]: command name (fixed: NT51950BASED_NORMAL_MODE)
...
Basic syntax:
```
./Combiner.exe NT51950BASED_NORMAL_MODE <CRC_method> <output_bin> <fw_bin> <block1_bin> <block1_source_address> <block1_destination_address> <block1_length> <block2_bin> <block2_source_address> <block2_destination_address> <block2_length> ...
```

Example:
```
./Combiner.exe NT51950BASED_NORMAL_MODE CRC8 "output.bin" "fw.bin" "block1.bin" 0x0 0x100 16 "block2.bin" 0x0 0x200 64 "block3.bin" 0x0 0x1000 32
```
