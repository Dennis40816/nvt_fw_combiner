...
- The only permitted code/reference snapshots are `gen_flash_bin_v2/`, `ab_code_combiner/`, and `flashmap/`.
- Do not add NFCG TypeScript/JavaScript source, Node packages, submodules, firmware BIN files, expected outputs, caches, virtual environments, or executables except the owner-approved `flashmap/IC FlashMap/combiner_1.13.0/Combiner.exe` evidence copy.
