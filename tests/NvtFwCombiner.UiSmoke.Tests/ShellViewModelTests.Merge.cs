...
public sealed partial class MergeWorkflowTests
{
    ...
    public void GeneralMergeUsesEditableMappingsAndOwnOutputLength()
    {
        ...
        Assert.Equal("Waiting for DP BIN", Assert.Single(viewModel.Merge.MergeMemoryRows).AfterSource);
        viewModel.SelectedLanguage = "Traditional Chinese";
        Assert.Equal("等待 DP BIN", Assert.Single(viewModel.Merge.MergeMemoryRows).AfterSource);
        Assert.Equal("等待 DP BIN", Assert.Single(viewModel.Merge.MergeCoverageSegments).SourceLabel);
