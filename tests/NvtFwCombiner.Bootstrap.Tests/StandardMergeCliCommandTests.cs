...
public sealed class StandardMergeCliCommandTests
{
    ...
    /// <summary>Selector-hidden Standard Merge profiles remain compiled internally but cannot be invoked from CLI.</summary>
    [Theory]
    [InlineData("NT51920")]
    [InlineData("51920")]
    ...
    [InlineData("NT51930")]
    [InlineData("51930")]
    [InlineData("nt51930-standard-merge-flashmap")]
    [InlineData("NT51931")]
    [InlineData("51931")]
    [InlineData("nt51931-standard-merge-gen-flash")]
    public async Task StandardMergeRejectsSelectorHiddenProfiles(string profileSelector)
    {
        CliRunResult result = await RunCliAsync([
            "standard-merge",
            "preview",
        ...
        Assert.Contains($"unknown standard merge profile '{profileSelector}'", result.Error, StringComparison.Ordinal);
        Assert.DoesNotContain("--dp is required", result.Error, StringComparison.Ordinal);
    }
