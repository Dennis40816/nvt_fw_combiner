...
{
    /// <summary>Checks canonical routing and milestone structure without freezing planning prose.</summary>
    [Fact]
    public void NfcRoadmapHasOneOrderedVersionAllocationEntryPoint()
    {
        string roadmap = ReadText("docs/architecture/nfc_roadmap.md");
        ...
        Assert.Contains(Version.Parse(ReadText("VERSION").Trim()), versions);
        Assert.Contains("## Explicit owner-unallocated queue", roadmap, StringComparison.Ordinal);
    }
