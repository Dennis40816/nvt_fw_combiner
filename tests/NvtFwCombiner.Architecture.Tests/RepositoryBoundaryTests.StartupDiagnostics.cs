...
public sealed partial class RepositoryBoundaryTests
{
    ...
    private static void AssertV0105PreloadBaselineIsFrozen()
    {
        ...
        Assert.Contains("256 gross removed and 289", baseline, StringComparison.Ordinal);
        Assert.Contains("98,074 to 98,107", baseline, StringComparison.Ordinal);
        Assert.Contains("full_production_ratchet=98_107", codeSizePolicy, StringComparison.Ordinal);
        ...
        (string Ticket, string Anchors, int Gross, int Added, int Net)[] ticketRows =
                 [
                     ...
                     ("PL-07 #379", "`WorkflowSessionPresentationViewModel.FirmwareInspection.cs:8-11,126-211,341-539`; `MergePresentationViewModel.General.cs:14,66-115`; `ReplacePresentationViewModel.General.cs:16,93-157` (375 nonblank candidate lines before retained result application is separated)", 250, 220, -30),
                      ("PL-00 #380", "`StartupTraceFileSink.cs`; `StartupTraceSession.cs`; `MainWindow.axaml.cs`; `ShellTextResources`; shared button roles; CtrlRAM relocalization; immutable Application DTO backing-field aliases; `scripts/measure-startup.ps1`", 256, 289, 33),
                 ];
